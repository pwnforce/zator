...
include "./rnn/GRU.circom";

// Template for an intermediary (backbone) Convnet layer
template Backbone(inDim) {
    // [running of hash outputted by the previous layer, hash of the activations of the previous layer]
    signal input step_in[2];

    signal input a_prev[inDim];
    signal input wz[inDim], wr[inDim], wx[inDim], wh[inDim];
    signal input bz[inDim], br[inDim];
    signal activations[inDim];
    component gru[inDim];
    ...
    gru[0].previous_hidden_state <== 0;
    for (var row = 1; row < inDim; row++) {
        gru[row].previous_hidden_state <== gru[row-1].current_hidden_state;
        activations[row] <== gru[row].current_hidden_state;
